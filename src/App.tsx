...
const App: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<
    "dashboard" | "history" | "search" | "profile"
  >("dashboard");
  ...
  const handleLogin = async (email: string, pass: string) => {
    ...
    // Admin Master: Diego
    if (
      (lowerEmail.includes("diego") || lowerEmail === "admin@floxhub.com") &&
      pass === "admin"
    ) {
      setUser({
        id: "1",
  ...
  if (!isLoggedIn) {
    return (
      <div className="min-h-screen bg-brand-dark flex items-center justify-center text-zinc-100">
          ...
          <div className="p-6 pb-32 animate-in fade-in">
            <div className="mb-8">
              ...
              <p className="text-zinc-500 text-[10px] font-black uppercase tracking-widest mt-1">
                Busca avançada no servidor GieM
              </p>
            </div>
            ...
            <div className="mt-12 text-center text-zinc-600">
              ...
              <p className="text-[10px] font-black uppercase tracking-[0.3em]">
                Aguardando termo para consulta
              </p>
            </div>
            <BottomNav
              activeTab={activeTab}
...
const BottomNav: React.FC<{
  ...
  onScanClick: () => void;
}> = ({ activeTab, onTabChange, onScanClick }) => {
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-50 px-6 pb-8 pt-4 bg-brand-dark/95 backdrop-blur-lg border-t border-brand-border flex justify-between items-center">
      <button
        ...
        className={cn(
          ...
          activeTab === "dashboard" ? "text-brand-primary" : "text-zinc-600",
        )}
      >
      ...
      <button
        ...
        className={cn(
          "flex flex-col items-center gap-1 px-3",
          activeTab === "history" ? "text-brand-primary" : "text-zinc-600",
        )}
      >
      ...
      <div className="relative -top-8">
        <button
          onClick={onScanClick}
          className="w-16 h-16 bg-gradient-to-tr from-orange-600 to-orange-400 rounded-full shadow-2xl shadow-orange-500/40 flex items-center justify-center text-white border-4 border-brand-dark transition-transform active:scale-90"
        >
      ...
      <button
        ...
        className={cn(
          "flex flex-col items-center gap-1 px-3",
          activeTab === "search" ? "text-brand-primary" : "text-zinc-600",
        )}
      >
      ...
      <button
        ...
        className={cn(
          "flex flex-col items-center gap-1 px-3",
          activeTab === "profile" ? "text-brand-primary" : "text-zinc-600",
        )}
      >
